...
public class UIrenderer	extends Gui {
	...
	public void draw() {
		...
		String xCoord = String.format("%.2f", mc.player.posX);
		String yCoord = String.format("%.2f", mc.player.posY);
		String zCoord = String.format("%.2f", mc.player.posZ);
		...
		for(Module module : Client.modulemanager.getEnabledModules()) {
			...
			Gui.drawRect(sr.getScaledWidth() - fr.getStringWidth(module.name) - 7, 2 + offset, sr.getScaledWidth() - fr.getStringWidth(module.name) - 5, 4 + fr.FONT_HEIGHT + offset, -1);
